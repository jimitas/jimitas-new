...
// フィードバック: documentレベルリスナー・コンポーネント外定義・userSelect:none
...
function handlePointerUp(e: PointerEvent) {
  ...
  // data-slot-idx を持つ要素にドロップされたか判定
...
export default function JuuTsukuriPage() {
  ...
  const dropToSlot = useCallback((card: Card, slotIdx: number) => {
    if (isFlashingRef.current) return
    if (fieldRef.current[slotIdx] !== null) return  // スロット占有済み
  ...
  // dropCallbackRef を最新の dropToSlot に同期
  useEffect(() => {
      ...
      <div className="flex items-center justify-between px-6 py-3 bg-white shadow-sm">
        ...
        {phase === "playing" && (
          <span className="text-lg text-gray-600">
            けした:{" "}
            <strong className="text-green-600">{cleared}</strong>ペア
          </span>
        )}
      </div>
      ...
      <div className="flex-1 flex flex-col p-4 gap-3 min-h-0">
        ...
        {phase === "clear" && (
          <div className="flex-1 flex flex-col items-center justify-center gap-8">
            <div className="text-center">
              ...
              <p className="text-4xl font-bold text-amber-600 mb-2">クリア！</p>
              <p className="text-2xl text-gray-600 mb-2">
                {cleared}ペア ぜんぶ けせたよ！
              </p>
              <p className="text-xl text-amber-500 font-bold">+5コイン ボーナス！</p>
            </div>
            <button
              onClick={startGame}
              className="px-12 py-5 bg-amber-500 hover:bg-amber-600 text-white text-3xl font-bold rounded-2xl shadow-lg transition-colors cursor-pointer"
        ...
        {phase === "gameover" && (
          <div className="flex-1 flex flex-col items-center justify-center gap-8">
            <div className="text-center">
              <p className="text-6xl mb-4">😢</p>
              <p className="text-4xl font-bold text-red-500 mb-2">ゲームオーバー</p>
              <p className="text-2xl text-gray-600 mb-2">
                ばが いっぱいで 10が つくれない！
              </p>
              <p className="text-xl text-gray-400">{cleared}ペア けせたよ</p>
            </div>
            <button
              onClick={startGame}
              className="px-12 py-5 bg-amber-500 hover:bg-amber-600 text-white text-3xl font-bold rounded-2xl shadow-lg transition-colors cursor-pointer"
                    ...
                    <div className="absolute inset-0 bg-indigo-300 rounded-xl border-2 border-indigo-500 flex items-center justify-center shadow hover:scale-105 transition-transform">
                      <div className="grid grid-cols-3 gap-0.5 opacity-50">
                        {Array.from({ length: 9 }, (_, k) => (
                          <div key={k} className="w-1.5 h-1.5 bg-indigo-800 rounded-full" />
                        ))}
                      </div>
                    </div>
                  ) : (
