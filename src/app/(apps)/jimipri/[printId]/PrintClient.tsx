...
// ======================================================
// じみぷり 各プリントの表示・操作（クライアントコンポーネント）
//
// 親（page.tsx・サーバーコンポーネント）から printId を受け取り、
// コントロールパネル（モード選択・もんだい・いんさつ）と
// A4プレビュー（印刷対象エリア）を描画する。
//
// 問題生成は純粋関数で行い、stateで保持 → JSXで描画する。
// 表示コンポーネントは _lib/components/ に分割済み。
//
// ※ もともと [printId]/page.tsx に "use client" で直書きしていたが、
//   generateStaticParams による静的化（CDNキャッシュ化）のため
//   サーバー/クライアントを分離した。printId は useParams ではなく
//   props で受け取る。
// ======================================================
...
export default function PrintClient({ printId }: { printId: string }) {
  ...
  if (!printDef) {
    return (
      <main className="max-w-5xl mx-auto px-4 py-8 text-center">
        ...
        <Link href="/jimipri" prefetch={false} className="text-brand-500 underline">
          じみぷりメニューへ戻る
        </Link>
      </main>
    )
