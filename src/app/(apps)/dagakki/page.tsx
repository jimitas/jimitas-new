"use client"

// ======================================================
// 打楽器ボード
//
// 西洋打楽器 3 + 和太鼓 5 の合計 8 種類の打楽器を、
// タップ/クリックで鳴らせる。マルチタッチ対応で複数人が同時に叩ける。
// 用途：音楽授業の「合いの手」「掛け声」「鑑賞時の体験」
// ======================================================

import { useState, useCallback } from "react"
import { useSound } from "@/hooks/useSound"

type Percussion = {
  id: string
  label: string
  reading: string
  emoji: string
  file: string
  color: "warm" | "brand" | "rose" | "amber" | "lime" | "sky" | "fuchsia" | "violet"
}

// 西洋打楽器
const PERCUSSION_WESTERN: Percussion[] = [
  { id: "kasuta", label: "カスタネット", reading: "Castanets", emoji: "🥁", file: "kasuta", color: "rose" },
  { id: "tam",    label: "タンバリン",   reading: "Tambourine", emoji: "🪇",  file: "tam",    color: "amber" },
  { id: "cow",    label: "カウベル",     reading: "Cowbell",    emoji: "🔔",  file: "cow",    color: "lime" },
]

// 和太鼓（5つの叩き方）
const PERCUSSION_JAPANESE: Percussion[] = [
  { id: "don",   label: "和太鼓・ドン",   reading: "DON",   emoji: "🥁", file: "don",   color: "warm" },
  { id: "dodon", label: "和太鼓・ドドン", reading: "DODON", emoji: "🥁", file: "dodon", color: "warm" },
  { id: "ka",    label: "和太鼓・カ",     reading: "KA",    emoji: "🥁", file: "ka",    color: "brand" },
  { id: "kaka",  label: "和太鼓・カカ",   reading: "KAKA",  emoji: "🥁", file: "kaka",  color: "brand" },
  { id: "kako",  label: "和太鼓・カコ",   reading: "KAKO",  emoji: "🥁", file: "kako",  color: "violet" },
]

// 色ごとのクラス（Tailwind の動的クラスを完全展開する）
const COLOR_CLASSES: Record<Percussion["color"], string> = {
  warm:    "bg-warm-500 hover:bg-warm-600 active:bg-warm-700 border-warm-600",
  brand:   "bg-brand-500 hover:bg-brand-600 active:bg-brand-700 border-brand-600",
  rose:    "bg-rose-500 hover:bg-rose-600 active:bg-rose-700 border-rose-600",
  amber:   "bg-amber-500 hover:bg-amber-600 active:bg-amber-700 border-amber-600",
  lime:    "bg-lime-500 hover:bg-lime-600 active:bg-lime-700 border-lime-600",
  sky:     "bg-sky-500 hover:bg-sky-600 active:bg-sky-700 border-sky-600",
  fuchsia: "bg-fuchsia-500 hover:bg-fuchsia-600 active:bg-fuchsia-700 border-fuchsia-600",
  violet:  "bg-violet-500 hover:bg-violet-600 active:bg-violet-700 border-violet-600",
}

export default function DagakkiPage() {
  const { play } = useSound()
  const [pressed, setPressed] = useState<Set<string>>(new Set())

  const handleHit = useCallback((p: Percussion) => {
    play(`/sounds/percussion/${p.file}.mp3`, 0.8)
    // 視覚フィードバック（150ms ハイライト）
    setPressed(prev => new Set(prev).add(p.id))
    setTimeout(() => {
      setPressed(prev => {
        const next = new Set(prev)
        next.delete(p.id)
        return next
      })
    }, 150)
  }, [play])

  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-1">
        打楽器ボード
      </h1>
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
        ボタンを押すと音が鳴るよ。タブレットでは複数本指で同時に叩けます。
      </p>

      {/* ===== 西洋打楽器 ===== */}
      <section className="mb-8">
        <h2 className="text-base font-bold text-gray-700 dark:text-gray-200 mb-3 pb-2 border-b-2 border-brand-400">
          🌍 西洋の打楽器
        </h2>
        <div className="grid grid-cols-3 gap-3">
          {PERCUSSION_WESTERN.map(p => (
            <PercussionButton
              key={p.id}
              p={p}
              isPressed={pressed.has(p.id)}
              onHit={() => handleHit(p)}
            />
          ))}
        </div>
      </section>

      {/* ===== 和太鼓 ===== */}
      <section>
        <h2 className="text-base font-bold text-gray-700 dark:text-gray-200 mb-3 pb-2 border-b-2 border-warm-400">
          🎌 和太鼓（5つの叩き方）
        </h2>
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          {PERCUSSION_JAPANESE.map(p => (
            <PercussionButton
              key={p.id}
              p={p}
              isPressed={pressed.has(p.id)}
              onHit={() => handleHit(p)}
            />
          ))}
        </div>
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-3 text-center">
          💡 「ドン」は中央を強く、「カ」は縁を打つ音。組み合わせて祭囃子のリズムを作れるよ。
        </p>
      </section>
    </div>
  )
}

// -----------------------------------------------------
// 打楽器ボタン
// -----------------------------------------------------
function PercussionButton({
  p,
  isPressed,
  onHit,
}: {
  p: Percussion
  isPressed: boolean
  onHit: () => void
}) {
  return (
    <button
      onPointerDown={onHit}
      className={`
        ${COLOR_CLASSES[p.color]}
        text-white rounded-2xl border-2 shadow-md
        py-6 px-3 font-bold transition-all duration-100
        ${isPressed ? "scale-95 shadow-inner" : "hover:-translate-y-0.5 hover:shadow-lg"}
      `}
    >
      <div className="text-4xl mb-1">{p.emoji}</div>
      <div className="text-sm">{p.label}</div>
    </button>
  )
}
